...
End Function
Public Function fxnVerifyUser(rst As DAO.Recordset) As Boolean
...
Public Function fxnTableExists(TableName As String) As Boolean
...
Public Function fxnGroupObservers(strEventID As String) As String
...
Set rst = CurrentDb.OpenRecordset("SELECT xref_Event_Contacts.Event_ID, [First_Name] & chr(32) & [Last_Name] AS Name " _
        & "FROM tlu_Contacts " _
        & "INNER JOIN xref_Event_Contacts ON tlu_Contacts.Contact_ID = xref_Event_Contacts.Contact_ID " _
        & "GROUP BY xref_Event_Contacts.Event_ID, [First_Name] & chr(32) & [Last_Name] " _
        & "HAVING (((xref_Event_Contacts.Event_ID) =" & """" & strEventID & """" & "));")

Do Until rst.EOF
    strObserver = strObserver & "    " & rst!Name
 
    
    rst.MoveNext
Loop

strObserver = LTrim(strObserver)
fxnGroupObservers = strObserver

Set rst = Nothing

End Function

Public Function fxnREPORTParkDensity(sqlReport As String, dbsReport As DAO.Database)


Dim rsData As Recordset
Dim rsSE As Recordset
Set rsData = dbsReport.OpenRecordset("qxtab_Park_Density")
Set rsSE = dbsReport.OpenRecordset("qxtab_Park_Density_SE")
Dim i As Integer
